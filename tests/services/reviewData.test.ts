...
describe('InMemoryReviewDataService', () => {
  ...
  describe('Session operations', () => {
    ...
    test('updateSession should throw error for non-existent session', async () => {
      await expect(service.updateSession('non-existent-id', { completed: true })).rejects.toThrow(
  ...
  describe('Item operations', () => {
    ...
    test('updateSessionItem should throw error for non-existent session', async () => {
      await expect(
        service.updateSessionItem('non-existent-id', testItem.id, { result: 'correct' })
    ...
    test('updateSessionItem should throw error for non-existent item', async () => {
      await expect(
        service.updateSessionItem(testSession.id, 'non-existent-item', { result: 'correct' })
